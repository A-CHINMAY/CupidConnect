	...
	return (
		<div className='flex flex-col h-screen bg-gradient-to-br from-blue-50 to-purple-50'>
			...
			<div className='flex-grow flex flex-col p-4 md:p-6 lg:p-8 overflow-hidden max-w-4xl mx-auto w-full'>
				<div className='flex items-center mb-4 bg-white rounded-lg shadow-md p-3 hover:shadow-lg transition-shadow duration-300 animate-fade-in'>
					<img
						src={match.image || "/avatar.png"}
						className='w-12 h-12 object-cover rounded-full mr-3 border-2 border-indigo-300 transition-transform duration-300 hover:scale-110'
					/>
					<h2 className='text-xl font-semibold text-gray-800'>{match.name}</h2>
				</div>

				<div className='flex-grow overflow-y-auto mb-4 bg-white rounded-lg shadow-md p-4 animate-slide-up'>
					{messages.length === 0 ? (
						...
						messages.map((msg) => (
							...
							>
								<span
									className={`inline-block p-3 rounded-lg max-w-xs lg:max-w-md transition-all duration-300 animate-fade-in ${msg.sender === authUser._id
...
const MatchNotFound = () => (
	<div className='h-screen flex flex-col items-center justify-center bg-gradient-to-br from-indigo-50 to-purple-100 bg-dot-pattern'>
		<div className='bg-white p-8 rounded-lg shadow-xl text-center transform hover:scale-105 transition-transform duration-300 animate-fade-in'>
			<UserX size={64} className='mx-auto text-indigo-500 mb-4 animate-pulse' />
			...
			<Link
				to='/'
				className='mt-6 px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 transition-all 
				focus:outline-none focus:ring-2 focus:ring-indigo-300 inline-block hover:shadow-lg'
			>
				Go Back To Home
			</Link>
		</div>
...
const LoadingMessagesUI = () => (
	<div className='h-screen flex flex-col items-center justify-center bg-gradient-to-br from-indigo-50 to-purple-100'>
